...
class VOCVersatileDataset(torch.utils.data.Dataset):
    def __init__(self, image_root, split, target_size,
        ...
        if superpixel_root is not None:
            self.data_superpixels = [os.path.join(superpixel_root, x+'.png') for x in names]
            for src in self.data_superpixels:
                CHECK_EXISTS(src)
        else:
        ...
        if annotation_root is not None:
            self.data_labels = [np.array(VOC.get_annotation(os.path.join(annotation_root, x+'.xml'))) for x in names]
    ...
    def __getitem__(self, index):
        ...
        # img, lbl, segs, sp
        if lbl is not None:
            image_label = torch.zeros((21 if self.image_label_use_background else 20,), dtype=torch.int64)
            for L in lbl:
                image_label[L] = 1
